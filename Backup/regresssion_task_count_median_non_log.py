...
!!! Optimal Threshold needs to be chosen at EDA stage !! and apply to define control and treatment groups. !!!
"""
...
# Clip extreme duration outliers (1st and 99th percentiles)
...
# --- Step 4: Run Regression WITHOUT log transform ---
model = smf.ols(
    formula="""
...
# --- Optional: Threshold Search with no log ---
results = []
for threshold in range(5, 35):
    daily_agg["high_load"] = (daily_agg["task_count"] > threshold).astype(int)
    daily_agg["task_count_c"] = daily_agg["task_count"] - threshold
